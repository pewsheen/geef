...
test("media actions only paste and never submit the page", async () => {
  ...
  assert.match(contentScript, /SUPPORTED_MEDIA_TYPES/);
  assert.match(contentScript, /GEEF_INSERT_MEDIA/);
  assert.doesNotMatch(sidepanelScript, /previewSend|Sent media|submit,/);
